...
func (h *ReportHandler) CreateReport(w http.ResponseWriter, r *http.Request) {
	...
	res := &models.Report{
		...
		IsValidated:     false, // Initial state
	}
...
func (h *ReportHandler) ServeReportImage(w http.ResponseWriter, r *http.Request) {
	...
	if filename == "" {
		...
		id, _ := strconv.Atoi(idStr)
		res, err := h.Service.GetReport(id)
		if err != nil || res == nil || res.ImagePath == "" {
	...
	// Try checking folders for the report
	idStr := r.PathValue("id")
	path := filepath.Join("uploads", "reports", idStr, filename)
	if _, err := os.Stat(path); os.IsNotExist(err) {
		// Fallback to legacy path if id matches
...
func (h *ReportHandler) UploadReportImage(w http.ResponseWriter, r *http.Request) {
	...
	// Save into a subfolder for this report
	...
	// Update the main image path in DB
